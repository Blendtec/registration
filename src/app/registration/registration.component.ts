...
export class RegistrationComponent implements OnInit, AfterViewInit {
  ...
  constructor(private storeService:StoreService, private el:ElementRef, private route: ActivatedRoute, private cdRef:ChangeDetectorRef, private requestService:RequestService) {
  	}
 ...
 setDefaultInputValues() {
  ...
  ngOnInit() {
  	...
  	try {
  		this.fieldsByCountry = require('./fieldsByCountry.json');
	  	this.countries = require('./countries.json');
	  	this.states = require('./states.json');
  	} catch (e) {
  		console.log(e);
  	}
  ...
  getCountryFromLanguage(language:string): void {
  	...
  	if (this.fieldsByCountry) {
	  	for (let i = 0; i < this.fieldsByCountry.length; i++) {
	  		if (typeof this.fieldsByCountry[i] === 'object' && typeof this.fieldsByCountry[i]['language'] === 'string' && this.fieldsByCountry[i]['language'] === language) {
  ...
  getCountryFromCountry(country:string): void {
  	let self = this;
  	for (let i = 0; i < this.fieldsByCountry.length; i++) {
  		if (typeof this.fieldsByCountry[i] === 'object' && typeof this.fieldsByCountry[i]['theCountry'] === 'string' && this.fieldsByCountry[i]['theCountry'] === country) {
  			self.currentCountry = this.fieldsByCountry[i];
  			return;
  		}
  	}
  ...
  isOptionSelected(country: string) : boolean {
  	if (typeof this.currentCountry != 'undefined' && typeof this.currentCountry['theCountry'] === 'string') {
  ...
  isAllInfoCorrect() : boolean {
  	let self = this;
  	for (let field in this.currentCountry) {
  		if (typeof this.currentCountry[field] === 'object' && typeof this.currentCountry[field]['isRequired'] === 'boolean') {
  			let out = self.isElementErrored(field);
  			if (out) {
  				return false;
  			}
  		}
  ...
  submitRegistration() : void {
  	...
  	if (this.isAllInfoCorrect()) {
  		let postObject = {};
  		this.inputValueByName['purchase_date'] = this.inputValueByName['purchase_year'] + "-" + this.inputValueByName['purchase_month'] + "-" + this.inputValueByName['purchase_day'];
  ...
  updateDays() : void {
  	if (this.inputValueByName['purchase_month'] && this.inputValueByName['purchase_year']) {
  		this.daysInMonth = [];
  		let numDays = new Date(this.inputValueByName['purchase_year'], this.inputValueByName['purchase_month'], 0).getDate();
  ...
  enableOtherBox() : void {
  	if (this.inputValueByName['purchase_place'] === 'Other Physical Retailer' || this.inputValueByName['purchase_place'] === 'Other Online Retailer') {
